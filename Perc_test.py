...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  8 16:12:04 2019
...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  8 15:13:46 2019
...
#mlp = MLPClassifier(activation='relu',solver='lbfgs',max_iter=1, alpha=1e-4, hidden_layer_sizes=(10,10,10,100), random_state=1) # 100
    ...
    for i in range(r):
        for j in range(r):
            for a in range(r):
                for u in range(r):
                    for h in range(r):
                        mlp = MLPClassifier(activation='relu',solver='lbfgs',max_iter=h+1, alpha=1e-4, hidden_layer_sizes=(i+1,j+1,a+1), random_state=u+1)
                        mlp.fit(x_norm, y)
                        C = (mlp.score(x, y, sample_weight=None))*100
                        if C > 80:
                            print('\nThe model score is {0}% int1 {1} int2 {2} int3 {3} int4 {4} int5 {5} \n'.format(C,i+1,j+1,a+1,h+1,u+1)) 
